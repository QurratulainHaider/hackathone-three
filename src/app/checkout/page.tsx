...
export default function CheckoutForm() {
  ...
  useEffect(()=>{
    ...
    const cart = data && Array.isArray(JSON.parse(data)) ? JSON.parse(data) : []
    setCartItem(cart)
    
    const ShipmentData = localStorage.getItem("ShipmentData")
    const shipData = ShipmentData ? JSON.parse(ShipmentData) : { shipment_cost: { amount: 0 } }
    const shipCost = parseFloat(shipData.shipment_cost.amount.toFixed(2))
  ...
  const totalAmount = Number(cartItem.reduce((acc: number, item: ICart) => acc + Number(item.price * item.quantity), 0)) + Number(shipCost)
      ...
      <div className="mx-auto grid max-w-6xl gap-6 md:grid-cols-2">
        ...
        <div className="space-y-6">
            ...
            <div className="grid gap-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <Select>
                  ...
                  <SelectContent>
                    {['Delhi', 'Maharashtra', 'Karnataka'].map((state) => (
          ...
          <div className="space-y-4">
            ...
            <Button className="w-full rounded-full py-6" disabled={cartItem.length === 0}>
              Continue
            </Button>
          </div>
        </div>
        ...
        <Card className="h-fit p-6">
          ...
          <div className="mt-4 space-y-4">
            ...
            <div className="flex justify-between text-sm text-gray-600">
              ...
              <span>{shipCost ? "₹ " + shipCost.toFixed(2) : "Free"}</span>
            </div>
            ...
            <div className="space-y-4 pt-6">
              {cartItem.length === 0 && <p className="text-gray-500">Your cart is empty</p>}
